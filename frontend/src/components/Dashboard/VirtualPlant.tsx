...
const VirtualPlant: React.FC<VirtualPlantProps> = ({ percentage }) => {
  // Determine plant state based on percentage
  let state = 'seed';
  if (percentage >= 80) state = 'bloom';
  else if (percentage >= 50) state = 'tree';
  else if (percentage >= 20) state = 'sprout';
  ...
  const states = {
    seed: {
      ...
      animation: { y: [0, 2, 0], transition: { repeat: Infinity, duration: 3 } }
    },
    sprout: {
      emoji: '🌿',
      ...
      animation: { rotate: [-2, 2, -2], transition: { repeat: Infinity, duration: 4, ease: "easeInOut" } }
    },
    tree: {
      emoji: '🌳',
      ...
      animation: { scale: [1, 1.05, 1], transition: { repeat: Infinity, duration: 3, ease: "easeInOut" } }
    },
    bloom: {
      emoji: '🌺',
  ...
  const currentState = states[state as keyof typeof states];
      ...
      <div className="h-32 w-32 flex items-center justify-center bg-surface/50 rounded-full mb-4 shadow-inner relative">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            animate={{ opacity: 1, y: 0, scale: 1, ...currentState.animation }}
