...
import { BarChart3, TrendingUp, Droplets, Calendar, Loader2, Clock } from 'lucide-react';
...
const Analytics = () => {
  ...
  const CustomTooltip = ({
    ...
    if (active && payload && payload.length) {
      return <div className="glass-panel !p-3 !rounded-xl !shadow-lg border-border/50">
  ...
  if (loading) {
    return <div className="flex h-full items-center justify-center min-h-[400px]">
        <Loader2 className="animate-spin text-primary" size={48} />
      </div>;
  }
  return <div className="max-w-6xl mx-auto space-y-8 pb-8">
      <motion.div initial={{
      opacity: 0,
      y: -20
    }} animate={{
      ...
      y: 0
        ...
        <div className="flex bg-surface p-1 rounded-xl border border-border shadow-sm w-max">
          {filters.map(f => <button key={f} onClick={() => setTimeFilter(f)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 ${timeFilter === f ? 'bg-primary text-white shadow-md' : 'text-text-secondary hover:text-text-primary hover:bg-surface-hover'}`}>
              {f}
            </button>)}
        </div>
      </motion.div>
      ...
      delay: 0.2
        ...
        <div className="h-[350px] w-full mt-4">
          <ResponsiveContainer width="100%" height="100%">
            {timeFilter === 'Month' ? <AreaChart data={monthlyData} margin={{
            top: 10,
            right: 10,
            left: -20,
            bottom: 0
              ...
              strokeDasharray: '4 4'
                ...
                <Area type="monotone" dataKey="amount" stroke="var(--color-primary)" strokeWidth={3} fillOpacity={1} fill="url(#colorAmount)" activeDot={{
              r: 6,
              strokeWidth: 0,
              fill: 'var(--color-primary-dark)'
            }} />
              </AreaChart> : <BarChart data={weeklyData} margin={{
            top: 10,
            right: 10,
            left: -20,
            bottom: 0
              ...
              opacity: 0.4
                ...
                <Bar dataKey="amount" radius={[6, 6, 0, 0]}>
                  {weeklyData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.amount >= entry.goal ? 'var(--color-primary)' : 'var(--color-primary-light)'} />)}
                </Bar>
              </BarChart>}
          </ResponsiveContainer>
        </div>
      ...
      <motion.div initial={{
        ...
        delay: 0.3
      }} className="glass-panel p-6 h-full mt-8">
        ...
        <div className="flex gap-4 overflow-x-auto pb-4 custom-scrollbar items-center">
          {todayLogs.map((log, i) => {
            ...
            return <div key={log.id || i} className="flex-none w-48 relative flex flex-col group">
          ...
          {todayLogs.length === 0 && <div className="text-center text-text-secondary text-sm py-4 italic w-full">
